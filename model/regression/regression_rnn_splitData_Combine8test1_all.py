...
for left_out in range(1, 10):
    ...
    # for left_out test set
    dataset_path = f"../../dataset/dataset_0725/p{left_out}"
    os.makedirs(dataset_path, exist_ok=True)

    # 加载数据
    X_train_tmp = np.load(f'{dataset_path}/X_train.npy')
    y_train_tmp = np.load(f'{dataset_path}/y_train.npy').astype(float).flatten()
    X_val = np.load(f'{dataset_path}/X_val.npy')
    y_val = np.load(f'{dataset_path}/y_val.npy').astype(float).flatten()
    X_test = np.load(f'{dataset_path}/X_test.npy')
    y_test = np.load(f'{dataset_path}/y_test.npy').astype(float).flatten()
    ...
    # 将加载的数据添加到列表中
    X_val_list.append(X_train_tmp)
    y_val_list.append(y_train_tmp)
    ...
    saveModel_dir = f'../../savedModel/combined8split1_leftout-{left_out}_tmp'
    os.makedirs(saveModel_dir, exist_ok=True)
    results_dir = f'../../saveResult/combined8split1__leftout-{left_out}_tmp'
    os.makedirs(results_dir, exist_ok=True)
    saveplot_dir = f'../../savePlot/combined8split1__leftout-{left_out}_tmp'
